...
public class SvarogTest {
	...
	@Test
	public void testOracleTimestampIssue() {
		SvReader dbu = null;
		if (SvConf.getDBType().equals("ORACLE"))
			try {
				dbu = new SvReader();
				String sql = "SELECT tbl0.PKID as tbl0_PKID,tbl0.META_PKID as tbl0_META_PKID,tbl0.OBJECT_ID as tbl0_OBJECT_ID,tbl0.DT_INSERT as tbl0_DT_INSERT,tbl0.DT_DELETE as tbl0_DT_DELETE,tbl0.PARENT_ID as tbl0_PARENT_ID,tbl0.OBJECT_TYPE as tbl0_OBJECT_TYPE,tbl0.STATUS as tbl0_STATUS,tbl0.USER_ID as tbl0_USER_ID,tbl0.PKID as tbl0_PKID,tbl0.REPO_NAME as tbl0_REPO_NAME,tbl0.TABLE_NAME as tbl0_TABLE_NAME,tbl0.SCHEMA as tbl0_SCHEMA,tbl0.SYSTEM_TABLE as tbl0_SYSTEM_TABLE,tbl0.REPO_TABLE as tbl0_REPO_TABLE,tbl0.LABEL_CODE as tbl0_LABEL_CODE,tbl0.USE_CACHE as tbl0_USE_CACHE,tbl0.CACHE_TYPE as tbl0_CACHE_TYPE,tbl0.CACHE_SIZE as tbl0_CACHE_SIZE,tbl0.CACHE_EXPIRY as tbl0_CACHE_EXPIRY,tbl0.IS_CONFIG_TABLE as tbl0_IS_CONFIG_TABLE,tbl0.CONFIG_UNQ_ID as tbl0_CONFIG_UNQ_ID FROM vsvarog_tables tbl0";

				Connection conn = dbu.dbGetConn();
				PreparedStatement st = null;
				ResultSet rs = null;
				try {
					DateTime dt = new DateTime();
					String sqldt = "select to_char(systimestamp) from dual";
					st = conn.prepareStatement(sqldt);
					rs = st.executeQuery();
					// System.out.println("After exec " + new
					// DateTime().toString());
					while (rs.next()) {
						sqldt = rs.getString(1);
					}
					// System.out.println("After full fetch" + new
					// DateTime().toString());
					System.out.println("Sys timestamp:" + sqldt);

					rs.close();
					st.close();

					System.out.println("Start 10 queries no datetime:" + new DateTime().toString());
					...
					System.out.println("End 10 queries no datetime:" + new DateTime().toString());
					// obj =

					sql = sql + " WHERE ( (tbl0.DT_DELETE = ?))";
					for (int i = 0; i < 10; i++) {

						st = conn.prepareStatement(sql);
						st.setObject(1, new Timestamp(((DateTime) SvConf.MAX_DATE).getMillis()));

						// System.out.println("Before exec " + new
						// DateTime().toString());
						rs = st.executeQuery();
						// System.out.println("After exec " + new
						// DateTime().toString());
					...
					System.out.println("End 10 queries WITH datetime:" + new DateTime().toString());
				...
				DbDataArray dbo = dbu.getObjectsByParentId(svCONST.OBJECT_TYPE_TABLE, svCONST.OBJECT_TYPE_FIELD, null,
						1, 0);

				DbQueryObject query = new DbQueryObject(SvCore.repoDbt, SvCore.repoDbtFields,
						SvCore.getDbt(svCONST.OBJECT_TYPE_TABLE), SvCore.getFields(svCONST.OBJECT_TYPE_TABLE), null,
						null, null);
				DbDataArray objects = dbu.getObjects(query, null, null);
	 ...
	 * (SvConnTracker.hasTrackedConnections()) fail(
	 ...
	 * 
	 * } if (SvConnTracker.hasTrackedConnections()) fail(
	 * "You have a connection leak, you dirty animal!");
	 * 
	 * }
	 */
